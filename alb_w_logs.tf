locals {
  create_with_logging = "${var.create && var.logging_enabled ? true : false}"
}
...
resource "aws_lb" "application" {
  ...
  count = "${local.create_with_logging ? 1 : 0}"
}
...
resource "aws_lb_target_group" "main" {
  ...
  count      = "${local.create_with_logging ? var.target_groups_count : 0}"
...
resource "aws_lb_listener" "frontend_http_tcp" {
  ...
  count             = "${local.create_with_logging ? var.http_tcp_listeners_count : 0}"
...
resource "aws_lb_listener" "frontend_https" {
  ...
  count             = "${local.create_with_logging ? var.https_listeners_count : 0}"
...
resource "aws_lb_listener_certificate" "https_listener" {
  ...
  count           = "${local.create_with_logging ? var.extra_ssl_certs_count : 0}"
}
